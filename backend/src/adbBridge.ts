...
export class ADBBridge extends EventEmitter {
  ...
  private checkAdbAvailable(): boolean {
    if (this.adbAvailable !== null) return this.adbAvailable;
    try {
      execSync("adb version", { stdio: "ignore", timeout: 3000 });
      this.adbAvailable = true;
    } catch {
      this.adbAvailable = false;
      console.log("[ADB] adb binary not found — ADB features disabled (optional)");
    }
  ...
  async connectDevice(host: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`[ADB] Failed to connect to ${host}:`, msg);
      return false;
    }
  }

  get isAvailable(): boolean {
    return this.checkAdbAvailable();
  }

  startPolling(host: string, intervalMs = 5000): void {
    const deviceId = `${host}:5555`;
    if (this.pollingIntervals.has(host)) return;
    console.log(`[ADB] Starting media polling for ${deviceId}`);
    ...
    const poll = async () => {
      try {
        ...
        if (info) {
          ...
          if (changed) {
            console.log(`[ADB] Media update from ${host}: ${info.packageName} "${info.title}" (${info.playbackState})`);
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[ADB] Poll error for ${host}:`, msg);
      }
    };
  ...
  private async getMediaSession(
    ...
    try {
      ...
      if (!output || output.length < 20) {
        console.log(`[ADB] Empty/short dumpsys output for ${deviceId} (${output.length} chars)`);
        return null;
      }
      const result = this.parseMediaSession(output);
      if (!result && output.includes("active=true")) {
        console.log(`[ADB] Active session found but parsing returned null for ${deviceId}`);
        const snippet = output.substring(
          Math.max(0, output.indexOf("active=true") - 200),
          output.indexOf("active=true") + 500
        );
        console.log(`[ADB] Snippet: ${snippet.substring(0, 400)}`);
      }
      return result;
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[ADB] Shell command failed for ${deviceId}:`, msg);
      return null;
    }
  }
